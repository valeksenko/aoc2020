...
execOp (mask, s) (Mem (addr, register)) = setRegister applyMask
    where
        setRegister r = (mask, M.insert addr r s)
        applyMask = foldr apply register . zip [0..] . reverse $ T.unpack mask
        apply (i, e) r = case e of
            '0' -> r `clearBit` i
            '1' -> r `setBit` i
            'X' -> r
